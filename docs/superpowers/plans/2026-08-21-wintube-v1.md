...
- [ ] **Step 1: Create solution, projects, and config files**
...
- [ ] **Step 4: Implement `Clients` and `InnerTubeClient`**
...
- [ ] **Step 3: Implement `VisitorDataStore`**
...
- [ ] **Step 3: Implement `StreamService`**
...
- [ ] **Step 1: Create the WinUI project files**
...
- [ ] **Step 2: Write the spike page (throwaway)**
...
- [ ] **Step 4: Implement `VideoItemParser`**
...
- [ ] **Step 3: Implement `FeedService` and `SearchService`**
...
- [ ] **Step 3: Implement `WatchProgressStore`**
...
- [ ] **Step 1: Implement `Session`** (code above, verbatim).
...
- [ ] **Step 1: Implement `VideoCard` and `HomePage`** per the above. Keep all InnerTube calls inside `Session.RunAsync`.

- [ ] **Step 2: Replace the window shell** — `MainWindow.xaml` becomes a `NavigationView` with a content `Frame`; selection navigates between `HomePage`, `SearchPage` (placeholder `Page` until Task 17), `HistoryPage` (placeholder until Task 18). Sign-out: `App.Session.SignOut()` then navigate to `LoginPage` and clear the back stack.
...
- [ ] **Step 1: Implement `PlayerPage`** per the above; wire `HomePage` card clicks to navigate with the `VideoItem`.

- [ ] **Step 2: Delete the spike page** and the `SpikePage` navigation fallback in `MainWindow`/`LoginPage`.
...
- [ ] **Step 1: Implement `SearchPage`** per the above.
...
- [ ] **Step 1: Implement `HistoryPage`** per the above.
